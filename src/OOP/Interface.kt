...
interface Shape1 { // Public by default
...
data class Circle11(val radius: Double) : Shape {
    override fun area(): Double = Math.PI * radius * radius
}

abstract class Rectangle1(val width: Double, val height: Double) : Shape {
}


//class Triangle(size: Double) : Shape Error: Sealed types can only be extended in the same file
class Triangle11(size: Double) : Shape {
    override fun area(): Double = 0.0
}

//if Rectangle not open, it cannot be extended here or other file
class Triangle22(size: Double) : Rectangle1(size, size) {
